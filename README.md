...
这些 Twitter/X 凭据必须来自同一个已绑定 Project 的 Developer App。App 权限需要设置为 `Read and write`，修改权限后请重新生成 `Access Token` 和 `Access Token Secret`。脚本使用 OAuth 1.0a 用户上下文上传图片，并使用 Twitter/X API v2 发布推文；`Bearer Token` 和 OAuth 2.0 token 不能替代上面的 Access Token。
